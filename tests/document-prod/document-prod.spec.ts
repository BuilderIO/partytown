...
test('document-prod', async ({ page }) => {
  ...
  await expect(testLocationGet).toHaveText('/document-prod/');
  ...
  await expect(testCurrentScriptSrcLocation).toHaveText('/document-prod/current-script-src.js');
});
